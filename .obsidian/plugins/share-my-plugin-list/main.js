...
// src/setting/suggester/FileSuggest.ts
var import_obsidian2 = require("obsidian");

// src/setting/suggester/TextInputSuggest.ts
var import_obsidian = require("obsidian");

// src/setting/suggester/Suggest.ts
...
function addFundingElement(containerEl) {
  const donateELdiv = containerEl.createEl("div");
  donateELdiv.setAttribute("style", "text-align: center; margin-top: 50px;");
  ...
  donateELa1.setAttribute("style", "color: gray; style: italic; font-size: 10px;");
...
var ShareMyPluginSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Path of file to export").setDesc("").addSearch((cb) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Export format").setDesc("").addDropdown((dropDown) => dropDown.addOption("list", "List").addOption("table", "Table").setValue(this.plugin.settings.exportFileFormat || "list").onChange((value) => {
      this.plugin.settings.exportFileFormat = value;
      this.plugin.saveSettings();
    }));
    containerEl.createEl("h2", { text: "Advance settings" });
    new import_obsidian3.Setting(containerEl).setName("Debug mode").setDesc("verbose log in the console").addToggle((toggle) => {
...
var EN = {
  ...
  commandExportFile: "Export to file",
  ...
  getPlugins() {
    ...
    for (let name in originPlugins) {
      try {
        ...
        plugin.manifest["author2"] = plugin.manifest.author.replace(/<.*?@.*?\..*?>/g, "").trim();
        plugins[name] = plugin;
      } catch (e) {
        console.log(e);
      }
