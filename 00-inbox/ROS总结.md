...
在ROS中，回调通常是指当某个事件发生时（比如接收到消息、服务请求完成等），系统自动执行的函数。这些回调函数是ROS节点响应外部事件的方式。然而，外部事件源（event sources）并不直接以ROS回调的形式存在，但它们可以通过发布消息到ROS主题来触发一系列ROS回调。
...
# 相关工作
卡西尼等人。提出了 ROS 2 处理链的第一个响应时间分析 [3]。唐等人。 [4]随后对独立线性处理链的特殊情况提供了更精确的分析，但代价是适用性更有限（即每个回调只能属于一个链）。因此，Tang 等人的分析 [4] 不适用于第 2 节中考虑的工作负载。 VI，其中包含分支处理链。