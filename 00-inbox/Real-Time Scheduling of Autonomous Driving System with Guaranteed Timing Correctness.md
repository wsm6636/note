...
1）对于任何每个任务执行模式固定的AD系统，如何检查AD任务系统的可调度性，以及如何估计AD系统中每个关键链的端到端延迟？ 
2）给定每个任务的几种执行模式候选，如何选择（执行模式的）精确组合以保证（1）和（9）的时序约束？
...
对于 τi 的任意作业 Ji,x 和非负整数 Z，我们令 Ji,x′ 为 τi 的作业，该作业恰好晚于 Ji,x 的释放 Z HP 时间单位，即 ri,x ′ = ri,x + Z HP。我们说 Ji,x′ 是从 Ji,x 向前翻译 Z 个超周期，可写为 Ji,x′ = Ji,x ⊕ Z HP 或同样地，Ji,x = Ji,x′⊖ Z HP。翻译作业的属性由以下引理给出。

**引理2**. 对于任何作业 Ji,x 及其翻译作业 Ji,x′ = Ji,x ⊕ Z HP，di,x′ = di,x + Z HP 且 ri,x′ = ri,x + Z HP。
证明。从平移运算“Ji,x ⊕ Z HP”的定义中导出这个引理是微不足道的。

**引理3**. 作业 Ji,x 是R (τi) 的就绪作业,当且仅当 Ji,x ⊕ Z HP 是 R(τi) 的就绪作业。
证明。不失一般性，我们假设 Ji,x ∈ R(τi)，并且我们将证明 Ji,x′ = Ji,x ⊕ Z HP 也属于 R(τi)。我们知道 ri,x′ − ri,x = Z HP 并且由于 Ti|HP 如 (11) 所示，Ji,x 和 Ji,x′ 释放之间的持续时间长度为 KTi，其中K=Z(HP/Ti）是整数。根据τi的虚拟周期Ti的定义，在就绪作业Ji,x释放后，每隔Ti时间单位释放一个就绪作业。因此，Ji,x′ 是 τi 的现成工作。类似的道理，如果Ji,x′ ∈ (τi)，我们也可以证明Ji,x ∈ (τi)。

![image.png](https://gcore.jsdelivr.net/gh/wsm6636/pic/202405081223125.png)

对于任何作业链 κ，我们使用 κ′ = κ ⊕ ZHP 来表示作业链，使得 κ 和 κ′ 具有相同数量的作业，即 |κ′| = |κ|，并且对于每个 k = 1、···、|κ|，κ′ 的第 k 个作业是从 κ 的第 k 个作业 Ji,x 向前翻译 Z 个超周期，即 Ji ,x′ = Ji,x ⊕Z HP，如图 7 所示。基于 Lem。 3、推导出翻译作业链的性质。
