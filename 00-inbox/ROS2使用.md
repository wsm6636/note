...
我尝试使用“chrt，“trace-cmd”，“ros2 trace”等方式追踪ros2程序的运行。并改变线程的调度策略和优先级。从我目前的结果来看，我觉得非实时内核下对ros2程序没有什么影响。

我编写的简单ros2测试程序，两个节点由两个执行器运行，他们分别持有一条回调链（定时器-回调1-回调2）。

在正常情况下他们会按顺序执行，就像这样（回调执行的时间图片）

当我用“chrt”时，无论是改为FIFO，DDL都没有产生影响。（和上图一样）

kernelshark输出的结果也是这样的。没有明显的区别。（优先级改变了）

我想这可能是以下原因：
1，非实时内核影响不大
2,   “DEADLINE”策略只有一种优先级就是0，我们不能通过命令改变（michele证实了这一点）。命令行方式改变程序不是很有效。
3, 内核的调度策略对ros2程序本身回调链并没有影响

或许我还有其他方面没有考虑到。但我不知道如何继续测试，您能给我一些建议吗
