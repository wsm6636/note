...
**引理 1**. 对于回调在同一 CPU 上的链 Γc := [τ1, ...τi, ..., τj, ..., τN ]，保证先前的链实例在新的链实例之前完成其执行。如果满足以下条件，实例将开始执行：(1) τj 的回调优先级高于 τi (j > i)，并且 (2) τj 运行在与 τi 的执行器优先级相同或更高的执行器上。
