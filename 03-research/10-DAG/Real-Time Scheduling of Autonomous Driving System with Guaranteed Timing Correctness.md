...
# 六．评估
我们对合成多速率 DAG 进行实验，每个 DAG 有 n 个节点。每个 DAG 的拓扑由 TGFF 工具生成 [14]。我们部分采用了 BOSCH 针对 2015 年 WATERS 挑战提出的详细汽车基准[10]。多速率周期任务集和关键链是随机生成的，同时符合表征。任务周期是根据给定的分布从汽车应用中的周期 [1,5,10,20,100,200,1000]ms 中选择的。对于每个 DAG，我们最多生成 15 个关键链，生成的关键链包含 1、2 或 3 个不同周期的任务，其中同一周期的任务可以出现 2 到 5 次。对于每个关键链π，π的数据年龄和反应时间的界限分别设置为Bπda = αda|π|HP和Bπrt = αrt|π|HP，其中αda和αrt都是系数。在我们的实验中，我们假设有3个处理器，因此，我们将多速率DAG的节点随机分为三部分。每个部分都静态分配给一个处理器。一旦节点被分配到特定的处理器，该节点释放的作业必须在该处理器上执行，并且不能迁移。对于每个任务 τi，我们令 ei/Pi 为 τi 的利用率，并令 u 为处理器上分配的任务的总利用率。对于给定的利用率 u，我们基于 UUniFast [15] 生成每个任务的 WCET。

我们通过 Python 3.7 实现我们的方法，并调用 Gurobi 工具 [16] 的例程来求解我们的 ILP 模型。该代码在配备 Intel Core i5-11500 CPU、2.7GHz、8G RAM 的 PC 上运行。在我们的实现中，我们为每个任务 τi 生成候选执行模式，如下所示。如果 Pi ≤ HP/5，我们将 τi 的虚拟周期设置为 Ti = 5Pi；否则，Ti = HP。我们枚举所有可能的 T i 向量，并且对于每个 Ti,x ∈ Ti，我们通过从 (ei, Ti,x) 范围中随机选择一个整数来设置 Di,x。如果我们的方法成功为每个任务选择了可行的执行模式，使得任务集是可调度的并且所有关键链的端到端延迟都有良好的界限，那么我们的方法将返回“succ”。否则，我们的方法将返回“failure”。

我们将所提出的方法与 Verucchi 等人提出的最新方法进行了比较 [7]。 Verucchi 方法（也称为基线方法）将多速率 DAG 转换为多个节点之间具有显式同步的单速率 DAG，然后选择转换后的具有合理端到端延迟的单速率 DAG 以非调度方式进行调度。 -先发制人的方式。如果所选 DAG（满足端到端延迟约束）通过了可调度性检查，则基线方法返回“succ”，否则，它无法满足目标多速率 DAG 的时序要求。据我们所知，在撰写本文之前，Verucchi 的工作 [7] 是唯一研究自动驾驶系统实时调度和分析的工作，重点关注所有数据时代、反应时间和多速率可调度性有向无环图模型。这就是我们选择论文[7]进行比较的原因。主要区别在于我们采用抢占式 EDF 调度算法，而 Verucchi 等人[7]针对多速率 DAG 采用非抢占式列表调度算法。由于自动驾驶系统具有很强的实时性要求，并且抢占式调度总体上比非抢占式调度具有更好的实时性，因此在自动驾驶系统中探索抢占式调度是有意义的。我们的实验工作表明，从保证实时性的角度来看，抢占式调度在AD系统中优于非抢占式调度。

我们用不同的参数组合进行实验，如图9所示。配置的值写在图标题中。对于每个数据点，都进行了 1000 次随机实验。我们评估了每种方法的成功率和计算时间。实验结果表明，我们的方法在所有场景下都优于基线方法，即我们的方法可以成功调度的多速率 DAG 实例的百分比比基线方法高 29.04%，并且我们的方法平均而言比基线方法快 6.31 倍。删除作业的任务总密度与原始任务系统的利用率非常接近。这表明我们的方法减少了很少的工作来保证AD系统的网络部分和物理部分所需的时序约束。
![image.png](https://gcore.jsdelivr.net/gh/wsm6636/pic/202405082032736.png)

图9（a）表明，当节点数量n增加时，两种方法的成功率都会增加。原因可能是，对于给定的关键链数量固定，随着DAG中节点总数n的增加，不同关键链共享的公共节点越来越少。这种情况下，不同关键链之间的干扰得到缓解，更容易保证关键链端到端的时延约束。当n增加时，基线方法的计算时间呈指数增加。相比之下，我们的方法总是可以在 6.28 秒内解决问题，并且随着 n 的增加没有显着变化。

图9（b）表明，当利用率u增加时，我们的方法的成功率增加。这是因为更大的利用率意味着每个单独任务 τi 的 WCET 更重，并且导致 τi 的每个就绪作业的截止日期更宽松。在这种情况下，任务集更有可能被成功调度，从而导致更高的成功率。当 u 增加时，两种方法的计算时间都会略有增加。我们的方法始终比基线方法快 6 倍。

图9（c）和（d）表明，当系数αda和αrt变大时，两种方法的成功率都会增加，因为反应时间和数据年龄（例如Bdπa和Brπt）的界限更加宽松。有了如此宽松的界限，端到端延迟约束就更容易满足，在这种情况下，分析方法总是在很少的迭代内解决问题。这就是为什么当 αda 和 art 增加时两种方法的计算时间都会减少。

# 七．相关工作

调度多速率 DAG，同时保证最坏情况的端到端延迟是一个具有挑战性的问题。主要有三种技术来解决这一挑战： 
(1) 一种有前途的方法是调整任务的沟通，以防止不利的重叠。这种方法的一个著名代表是逻辑执行时间（LET）范式[17]-[19]，它在任务激活时（和截止日期）执行任务数据的输入（和输出）。 LET 范式保证了确定性的执行和可预测的端到端延迟。秒中的时序分析。本文的V可以看作是基于LET范式的。
(2)另一种方法是在数据链的特定作业之间重新引入优先约束，这称为作业级依赖关系(JLD)。对多速率 DAG 中的 JLD 进行静态分析具有挑战性。提出了几种方法[7]、[20]、[21]将多速率 DAG 转换为单速率 DAG 以确定 HP 中的 JLD。贝克尔等人。 [22]提出了一种启发式方法来求解 JLD 以满足端到端时序要求。赛迪等人。 [20]确定保证不同激活率任务之间正确数据交换的JLD。韦鲁基等人。 [7]构建了多个单速率 DAG，并选择具有合理日期年龄和反应时间的 DAG。科登等人。 [21]通过将多速率 DAG 的每个作业复制固定次数来得出数据年龄的上限。尽管 JLD 可以提供更严格的端到端延迟上限，但它们需要对系统的实现进行重大更改，并导致大量的同步开销。克劳斯等人。 [8]主张在严格的数据期限保证和同步开销之间进行权衡。
(3) 可以通过调整作业的发布时间和截止日期等参数来避免由 JLD 引起的此类开销。该模式由[23]实现。戈哈里等人。 [3] 提出了多速率 DAG 的数据时代分析，其中任务的释放时间和执行时间存在不确定性。我们的方法删除了任务的冗余作业，这可以看作是任务周期调整的实现。所有现有工作都是在分析端到端延迟之后（或之前）分析多速率 DAG 任务集的可调度性，无法共同设计 AD 系统的网络和物理部分。本文提出了一种新颖的分析框架来共同保证任务集的可调度性和端到端的延迟约束。
# 八．结论
AD系统的一个基本问题是，为了简单性而牺牲了任务固有的数据依赖性，并且没有明确地实现为优先级约束，因此，具有不同激活率的任务之间可能存在复杂的数据依赖性，使得分析任务变得非常困难。 AD 系统的实时行为。本文提出了一种新颖的时序分析框架，共同保证AD系统网络和物理部分的正确性。在我们的分析框架下，可以自动应用“设计-再设计”过程。更重要的是，当前设计过程的失败可以用来指导重新设计过程，这可能有助于为AD系统开发有效的设计方法。
