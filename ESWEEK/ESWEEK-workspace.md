...
端到端时序约束根据端到端时序语义，包括反应时间约束和数据年龄约束，并且由AUTOSAR[AUTOSAR]定义。其中反应时间表示外部采样并更新数据直到系统每个相关任务处理这个更新的最早时间间隔的长度，如图所示；数据年龄则表示对于外部采样的更新数据开始处理后直到基于采样数据所产生激励之间的时间间隔长度。
