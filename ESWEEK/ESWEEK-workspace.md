...
另一种则是事件触发的方式，即任务链上的每个任务需要其前一个任务执行结束生成数据，随后触发该任务读取数据进行下一步处理。【7461359】
[18]计算了具有同步和异步通信的线程序列的响应时间，[19]使用只支付突发一次分析技术优化了穿越多个处理器的触发链的端到端延迟。
